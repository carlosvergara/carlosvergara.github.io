...
Hola muchachada. Hoy la palangana viene repleta de... informes, ha oído usted bien (soy así de aburrido).
...
library(knitr)
library(dplyr) ##  Si no la tuvieras instalada, anímate: merece la pena. 
set.seed(1)  ##  Así el ejemplo será reproducible.
example <- data.frame(trat = gl(3, 100,
                                labels = c('Trat. 1', 'Trat. 2', 'Trat. 3')),
                      chol = as.vector(sapply(c(200, 160, 210), function(x) {
                        rnorm(100, mean = x, sd = x/10)
                        })))
mi.summary.table <- example %>%  ##  Sintaxis de dplyr: en serio, prueba.
  ...
  summarise(N = n(),
            ...
            IQR = IQR(chol))
##  Tabla básica con la funcion kable de knitr.
kable(mi.summary.table, digits = 2, caption = 'Descriptiva según tratamiento')  
```
...
library(ggplot2)  ##  Si no la tuvieras instalada, anímate: merece la pena.
library(grid)
library(gridExtra)  ##  Para utilizar la función grid.arrange().
mi.box <- ggplot(example, aes(trat, chol)) + geom_boxplot() + theme_classic() +
  labs(x = 'Tratamiento', y = 'Colesterol (mg./dl.)', title = '') +
  scale_y_continuous(limits = c(120, 260), breaks = seq(120, 260, 20)) +
  theme(axis.title = element_text(size = 16, face = 'bold', family = 'Times'),
        axis.text = element_text(size = 12, face = 'plain', family = 'Times'),
        axis.title.y = element_text(vjust = 1),
        axis.title.x = element_text(vjust = 0))
mi.den <- ggplot(example, aes(colour = trat, x = chol)) + geom_line(stat = 'density') +
  theme_classic() + scale_x_continuous(limits = c(120, 260), breaks = seq(120, 260, 20)) +
  labs(x = 'Colesterol (mg./dl.)', y = 'Densidad', title = '', colour = 'Tratamiento') +
  theme(axis.title = element_text(size = 16, face = 'bold', family = 'Times'),
        axis.text = element_text(size = 12, face = 'plain', family = 'Times'),
...
grid.arrange(mi.box, mi.den, nrow = 1,
             main = textGrob('Colesterol según tratamiento',
                             gp = gpar(fontsize = 20, font = 2, fontfamily = 'serif')))
```
...
mi.aov <- aov(chol ~ trat, example)
par(mar = c(4, 4, 1, 1), bty = 'l', family = 'serif', mgp = c(2, 0.5, 0), xaxs = 'i', yaxs = 'i')
layout(matrix(c(1, 2, 1, 2), byrow = T, ncol = 2))
with(mi.aov, plot(fitted.values, residuals, xlab = 'Valores ajustados',
                  ylab = 'Residuos', cex = 1.5, cex.lab = 1.3, col = 4,
                  xlim = c(150, 215), ylim = c(-60, 60)))
abline(h = 0, lty = 2)
with(mi.aov, lines(lowess(fitted.values, residuals), col = 2))
with(mi.aov, qqnorm(residuals, xlab = 'Cuantiles teóricos', main = '', cex = 1.5, 
                    ylab = 'Cuantiles muestrales', cex.lab = 1.3, col = 4,
                    xlim = c(-3, 3), ylim = c(-60, 60)))
with(mi.aov, qqline(residuals, col = 2))
abline(h = 0, lty = 2)
summary(mi.aov)
```
...
```{r aov.tab}
##  Hay cambiar la clase del objeto mi.aov en una que kable pueda manejar.
mi.aov <- anova(mi.aov)
##  Ponemos los nombres de columnas y filas que nos parezca.
colnames(mi.aov) <- c('Grados de libertad', 'Suma de cuadrados', 'Media de cuadrados', 'Valor F', 'Valor p')
rownames(mi.aov) <- c('Nivel', 'Residuos')
kable(mi.aov, digits = 2, caption = 'Tabla del ANOVA')
```
